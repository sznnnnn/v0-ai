...
export function WriteDocumentPageClient({ programId }: { programId: string }) {
      ...
      <main className="px-4 py-6 sm:px-6 sm:py-8">
        {missingMatch && (
          <div className="rounded-lg border border-dashed border-border p-10 text-center">
            ...
            <p className="mb-4 text-sm text-muted-foreground">请先在匹配页生成结果，再把项目加入项目清单。</p>
        ...
        {!missingMatch && !missingProgram && addedIdsReady && notAdded && !hasLocalDraft && (
          <div className="rounded-lg border border-dashed border-border p-10 text-center">
            <p className="mb-4 text-sm text-muted-foreground">
              请先将该项目加入项目清单后再写文书；若已有草稿，可从匹配页对应项目的「文书」入口进入。
            </p>
